...
export default function TaskCard({
  ...
  onToggle, onEdit, onCancelEdit, onSaved, onDelete
}) {
  ...
  const handleDelete = (e) => {
    e.stopPropagation()
    // Si hay onDelete del padre (TasksPage), úsalo para el ConfirmModal
    if (onDelete) {
      onDelete()
    } else {
      // Fallback: eliminar directo (usado en CourseDetailPage)
      ...
      >
        <button
          ...
          style={{
            ...
            backgroundColor: form.status === "completada" ? currentStatus?.color : "transparent",
          }}
        >
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <span className={`text-gray-600 text-xs transition-transform duration-200 ml-1 ${isExpanded ? "rotate-180" : ""}`}>▾</span>
        </div>
      </div>
          ...
          {isEditing ? (
            ...
            <div className="flex flex-col gap-2">
              <p className="text-gray-500 text-sm">
                {form.description || <span className="text-gray-700 italic">Sin descripción</span>}
              </p>
              {form.dueDate && (
                <p className="text-xs text-gray-600">
                  Fecha límite: {new Date(form.dueDate + "T12:00:00").toLocaleDateString("es-ES", {
