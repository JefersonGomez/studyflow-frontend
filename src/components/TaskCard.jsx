...
// ← Agrega courseName a las props
export default function TaskCard({ task, courseId, courseName, onDelete, onUpdate }) {
  ...
  const isOverdue = form.dueDate && new Date(form.dueDate) < new Date() && form.status !== "completada";

  const updateMutation = useMutation({
    mutationFn: (data) => updateTask(task.id, data),
    onSuccess: () => {
      queryClient.invalidateQueries(["tasks", courseId]);
      queryClient.invalidateQueries(["all-tasks"]); // ← Invalidar también la vista global
      setEditing(false);
    },
  });
  ...
  return (
    <div className={`group bg-[#141414] border rounded-2xl transition-all duration-200 overflow-hidden ${
      form.status === "completada" ? "border-emerald-500/20 opacity-75" 
      ...
      <div className="flex items-center gap-3 px-4 py-3 cursor-pointer" onClick={() => !editing && setExpanded(!expanded)}>
        ...
        <button onClick={cycleStatus}
          ...
          style={{ borderColor: currentStatus?.color, backgroundColor: form.status === "completada" ? currentStatus?.color : "transparent" }}>
          {form.status === "completada" && (
            <svg width="10" height="8" viewBox="0 0 10 8" fill="none">
              <path d="M1 4l2.5 2.5L9 1" stroke="white" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
          )}
          {form.status === "en_progreso" && <div className="w-2 h-2 rounded-full bg-blue-400" />}
        </button>

        {/* Contenido central */}
        <div className="flex-1 min-w-0">
          <p className={`text-sm font-medium truncate transition-all duration-200 ${
            form.status === "completada" ? "line-through text-gray-500" : "text-white"
          }`}>
            {form.title}
          </p>
          
          {/* ✅ CORRECCIÓN: Mostrar nombre del curso si existe */}
          {courseName && (
            <p className="text-[10px] text-emerald-400/80 font-bold uppercase tracking-wider mt-0.5 truncate">
              📚 {courseName}
            </p>
          )}
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          <span className="text-xs px-2 py-0.5 rounded-full font-medium hidden group-hover:inline-flex"
            style={{ backgroundColor: currentStatus?.bg, color: currentStatus?.color }}>
            {currentStatus?.label}
          </span>

          {isOverdue && <span className="text-xs text-red-400 font-medium">Vencida</span>}

          {form.dueDate && !isOverdue && (
            <span className="text-xs text-gray-600">
              {new Date(form.dueDate + "T12:00:00").toLocaleDateString("es-ES", { day: "numeric", month: "short" })}
            </span>
          )}

          <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
            <button onClick={(e) => { e.stopPropagation(); setEditing(true); setExpanded(true); }}
              className="w-7 h-7 rounded-lg text-gray-500 hover:text-white hover:bg-[#252525] transition-colors flex items-center justify-center text-xs">✏️</button>
            <button onClick={(e) => { e.stopPropagation(); deleteMutation.mutate(); }}
              className="w-7 h-7 rounded-lg text-gray-500 hover:text-red-400 hover:bg-red-500/10 transition-colors flex items-center justify-center text-xs">🗑️</button>
          </div>

          <span className={`text-gray-600 text-xs transition-transform duration-200 ml-1 ${expanded ? "rotate-180" : ""}`}>▾</span>
        </div>
      </div>
            ...
            <div className="flex flex-col gap-3">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })}
                    className="w-full bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg px-3 py-2 text-white text-sm outline-none focus:border-emerald-500/50 transition-colors" autoFocus />
                </div>
                <div>
              ...
              <div className="flex gap-2 pt-1">
                ...
                <button onClick={() => { setEditing(false); setExpanded(false); }}
                  className="bg-[#1f1f1f] hover:bg-[#252525] text-gray-400 text-xs px-4 py-2 rounded-lg transition-colors">Cancelar</button>
              </div>
            </div>
          ) : (
            <div className="flex flex-col gap-2">
              <p className="text-gray-500 text-sm">{form.description || <span className="text-gray-700 italic">Sin descripción</span>}</p>
              {form.dueDate && (
                <p className="text-xs text-gray-600">Fecha límite: {new Date(form.dueDate + "T12:00:00").toLocaleDateString("es-ES", { day: "numeric", month: "long", year: "numeric" })}</p>
              )}
            </div>
          )}
        </div>
