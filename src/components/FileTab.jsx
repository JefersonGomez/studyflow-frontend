...
export default function FileTab({
  ...
  const handleAI = async (action, fileId) => {
    ...
    try {
      ...
      if (action === "studyplan")
        res = await generateStudyPlanByFile(fileId, studyDays);
      setAiResult(res.data);
       // ✅ NOTIFICAR AL PADRE SOLO SI FUE UN ANÁLISIS DE PDF EXITOSO
      if (action === "analyze" && res.data?.creados > 0 && onEventsCreated) {
        onEventsCreated();
      }
    } catch (err) {
      console.error(
      ...
      <div className="flex-1 min-w-0">
        ...
        >
          ...
          <div className="flex flex-col items-center justify-center gap-3">
            <div
              className={`w-14 h-14 rounded-2xl flex items-center justify-center text-2xl transition-transform duration-300 ${isUploading ? "animate-pulse bg-emerald-500/10" : "bg-[#1f1f1f] group-hover:scale-110 group-hover:bg-emerald-500/10"}`}
            >
              {isUploading ? "⏳" : ""}
            </div>
            <div>
              ...
              <p className="text-gray-500 text-sm mt-1">
                Arrastra o haz clic para subir. La IA extraerá el contenido
                automáticamente.
              </p>
            </div>
          </div>
          ...
          {loadingFiles ? (
            ...
            <div className="text-center py-12 bg-[#141414] rounded-2xl border border-[#1f1f1f]">
              <p className="text-gray-600">
                No hay archivos en este curso aún.
              </p>
            </div>
          ) : (
            files.map((file) => (
              ...
              >
                {/* Icono Archivo */}
                <div
                  className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 transition-colors ${selectedFile?.id === file.id ? "bg-emerald-500/10 text-emerald-400" : "bg-[#1f1f1f] text-gray-400"}`}
                >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <span
                      className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${file.aiProcessed ? "bg-emerald-500/10 text-emerald-400" : "bg-yellow-500/10 text-yellow-500"}`}
                    >
                      {file.aiProcessed ? "IA Listo" : "Pendiente"}
                    </span>
                    <span className="text-gray-600 text-xs">•</span>
                    <p className="text-gray-500 text-xs">
                      {new Date(file.createAt).toLocaleDateString("es-ES", {
                        day: "numeric",
                        month: "short",
                        year: "numeric",
                      })}
                ...
                <div className="flex items-center gap-3 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                  {selectedFile?.id === file.id && (
                    <span className="text-emerald-400 text-xs font-bold hidden sm:block">
                      Seleccionado
                    </span>
                  )}
                  <button
        ...
        <div className="sticky top-8 bg-[#141414] border border-[#1f1f1f] rounded-3xl p-6 shadow-xl">
          ...
          <div className="flex items-center justify-between mb-6 pb-4 border-b border-[#1f1f1f]">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-emerald-400 text-[10px] font-medium bg-emerald-500/10 px-2 py-0.5 rounded-full inline-block mt-0.5">
                  Ollama
                </p>
              </div>
            </div>
          ...
          {!selectedFile ? (
            <div className="flex flex-col items-center justify-center py-12 text-center space-y-3 opacity-50">
              <div className="w-16 h-16 rounded-full bg-[#1f1f1f] flex items-center justify-center text-2xl">
                👈
              </div>
              ...
              <div className="grid gap-3">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-gray-500 text-xs mt-0.5 leading-relaxed">
                        Extrae fechas de exámenes, quizzes y entregas
                        automáticamente.
                      </p>
                    </div>
                  </div>
                ...
                <div className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-xl p-4">
                  ...
                  <div className="flex items-center gap-3 mb-3 bg-[#111] rounded-lg p-2 border border-[#2a2a2a]">
                    ...
                    />
                    <span className="text-gray-500 text-xs border-l border-[#2a2a2a] pl-3 flex-1">
                      días de preparación
                    </span>
                  </div>
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="flex flex-col gap-2 max-h-60 overflow-y-auto pr-1 custom-scrollbar">
                            {aiResult.eventos?.map((e, i) => (
                              <div
                                key={i}
                                className="flex items-center justify-between bg-[#111] border border-[#1f1f1f] rounded-lg px-3 py-2.5 hover:border-[#333] transition-colors"
                              >
                                ...
                                <span
                                  ...
                                  style={{
                                    backgroundColor:
                                      FILE_TYPE_COLORS[e.tipo]?.bg || "#333",
                                    color:
                                      FILE_TYPE_COLORS[e.tipo]?.color || "#fff",
                                  }}
                                >
