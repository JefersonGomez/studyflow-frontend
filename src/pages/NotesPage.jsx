    ...
    <Layout>
      ...
      <div className="max-w-7xl mx-auto p-6 relative z-10">
        ...
        <div className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <h1 className="text-3xl font-black text-white tracking-tight mb-2">Mis Notas</h1>
            <p className="text-gray-400 text-sm">Tu biblioteca personal de apuntes.</p>
          </div>
        ...
        <div className={`transition-opacity duration-300 ${expandedId ? "opacity-20 pointer-events-none blur-[2px]" : "opacity-100"}`}>
          {filteredNotes.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-24 bg-[#141414] border border-dashed border-[#2a2a2a] rounded-2xl mt-8">
              ...
              <p className="text-gray-400 font-medium text-lg">{search ? "Sin coincidencias" : "Libreta vacía"}</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
              {filteredNotes.map((note, index) => {
                ...
                return (
                  <div key={`note-${note.id}`} className="animate-note-card" style={{ animationDelay: `${index * 50}ms` }}>
                    ...
                    >
                      <div className="flex justify-between items-start mb-3">
                        ...
                        <span className="text-[10px] text-gray-600">{new Date(note.createAt).toLocaleDateString()}</span>
                      </div>
                      <h3 className="text-white font-bold text-lg mb-2 line-clamp-1">{note.title}</h3>
                      <p className="text-gray-400 text-sm line-clamp-3 flex-1">{note.content}</p>
                    </div>
                  </div>
                );
              })}
      ...
      {expandedId && activeNote && (
        <div 
          ...
          onClick={() => { setExpandedId(null); setEditingId(null); }} // Cerrar al hacer click fuera
        >
          <div 
